PACKAGE  = stash.kopano.io/kgol/smtpst
...
dist: 3rdparty-LICENSES.md ; $(info building dist tarball ...)
	...
	cp -avf ../scripts/kopano-smtpstd.binscript "${PACKAGE_NAME}-${VERSION}/scripts" && \
	cp -avf ../scripts/kopano-smtpstd.service "${PACKAGE_NAME}-${VERSION}/scripts" && \
	cp -avf ../scripts/smtpstd.cfg "${PACKAGE_NAME}-${VERSION}/scripts" && \
